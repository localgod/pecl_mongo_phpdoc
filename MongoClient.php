...
class MongoClient {
	 ...
	 * tagsets (for ReplicaSet connections), FALSE otherwise. This property does not take authentication into
	 * account.
	 * @var boolean
	 */
	 ...
	 * this may be set to a string value (e.g. "recycled", "new") when persistent connections are used.
	 * @var mixed
	 */
	public $status = null;
	 ...
	 * @param boolean|string $connection (optional)  If connection is not given, or FALSE then connection that would be
	 ...
	 *
	 * @param string $serverHash   The server hash that has the cursor. This can be obtained through MongoCursor::info().
	 * @param int|\MongoInt64 $id  The ID of the cursor to kill. You can either supply an int containing the 64 bit
	 ...
	 *
	 * @param string $read_preference  The read preference mode: MongoClient::RP_PRIMARY, MongoClient::RP_PRIMARY_PREFERRED,
	 *                                 MongoClient::RP_SECONDARY, MongoClient::RP_SECONDARY_PREFERRED, or MongoClient::RP_NEAREST
	 * @param string $tags (optional)  An array of zero or more tag sets, where each tag set is itself an array of criteria
